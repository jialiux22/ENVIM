...
ENVIM = function(microbio.train,
  ...
  train.cor = c()
  test.cor = c()
  test.pred = data.frame(numeric(nrow(metab.test)))
  rownames(test.pred) = rownames(metab.test)
  name.gene.train = matrix(NA,ncol = ncol(metab.train),nrow = 10000)
  name.gene.test = matrix(NA,ncol = ncol(metab.train),nrow = 10000)
  colnames(name.gene.train) = colnames(metab.test)
  colnames(name.gene.test) = colnames(metab.test)
  ...
  for (col.n in 1:ncol(metab.train)) {
    ...
    y.test = metab.test[,col.n]
    
    names(y.train) = rownames(metab.train)
    names(y.test) = rownames(metab.test)
    ...
    test.x1 = as.matrix(cbind(rep(1,nrow(metab.test)),test.x1))
    ...
    ## testing correlation
    test.cor[col.n] = cor(pred.test,y.test,method = "spearman")
    
    ## testing prediction
    test.pred[,(col.n+1)] = pred.test
    mse.test[col.n] = mean((pred.test - y.test)^2)
  ...
  ## summary table
  sum.table <- data.frame(colnames(metab.test))
  colnames(sum.table)[1] = "Compound_name"
  sum.table$Train_spearman_cor = train.cor
  sum.table$Train_mse = mse.train
  sum.table$Test_spearman_cor = test.cor
  sum.table$Test_mse = mse.test
  ...
  print(paste0("Weight.train.csv, Weight.test.csv, Summary.csv are stored in ",outputdirectory))
